...
def load_data(root):
    if root.find("npy") != -1:
        ...
        for file in file_list:
            file_name = root + file
            csi = np.array(np.loadtxt(file_name,dtype=float,delimiter=','))
            csi = torch.from_numpy(csi).float().unsqueeze(0)  # 1*2000*3*30
            csi.requires_grad = False
            print(csi.shape)
            #csi = csi.view(-1,1, 2000, 90)
            data.append(csi)
...
if __name__=="__main__":
    ...
    dataset1 = torch.load('Data1.pt')
    aclist = ['bed', 'fall', 'pickup', 'run', 'sitdown', 'standup', 'walk']
    train_data_loader = data_loader(dataset)
    test_data_loader = data_loader(dataset1)
    args = get_args()
    net = Model.HARTrans(args).to(device)
    ...
    for ep in range(0, args.epoch):
        data_tqdm = tqdm(train_data_loader)
        # print("\nEpoch{}/{}".format(ep, n_epochs))
        for input_data in data_tqdm:
            ...
            在这里训练
            还需要
            1.优化器 (可以使用计划优化器)  ①optimizer.zero_grad()  ③optimizer.step()
            2.损失函数 torch.nn.CrossEntropyLoss （Adam或者是AdamW）  ②loss.backward()
            3.tensorboard   writer (可视化)
            """
            time_start = time.time()
